...
export function ProjectsPage() {
  ...
  const handleAddTask = async (bucketId: string, title: string, opts?: { assignee?: string; due_date?: string; priority?: number }) => {
  ...
  const handleCreateTaskFromModal = async (
    bucketId: string,
    title: string,
    patch: { description?: string | null; priority?: number; due_date?: string | null; assignee?: string | null }
  ) => {
    const task = await createTask({
      title,
      bucket_id: bucketId,
        ...
        buckets: prev.buckets
          ...
          .map(bwt =>
            bwt.bucket.id === destBucketId
              ? { ...bwt, tasks: [...bwt.tasks.slice(0, destIndex), { ...task, bucket_id: destBucketId, position }, ...bwt.tasks.slice(destIndex)] }
              : bwt
          ),
      };
    });
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Content */}
      <div className={`flex-1 min-h-0 ${viewMode === 'board' ? 'overflow-auto p-4' : 'overflow-auto p-4'}`}>
        {viewMode === 'board' && (
        ...
        {viewMode === 'list' && (
          <ListView board={board} onTaskClick={setSelectedTask} />
        )}
        {viewMode === 'table' && (
          <TableView board={board} onTaskClick={setSelectedTask} />
        )}
      </div>
      ...
      <TaskDetailDrawer
        ...
        onClose={() => { setSelectedTask(null); setCreateBucketId(null); }}
