...
interface Props {
  ...
  addTaskColor?: string;
  onAdd: (title: string, opts?: { assignee?: string; due_date?: string; priority?: number }) => Promise<void>;
}
...
function CalendarPopover({ value, onChange, onClose, anchor }: {
  value: string;
  ...
  const shortcuts = [
    ...
    { label: 'Tomorrow', sub: fmt(addDays(today, 1), { weekday: 'short' }), date: addDays(today, 1) },
      ...
      <div className="w-48 border-r border-stone-100 dark:border-neutral-800 py-2">
        {shortcuts.map(s => (
          <button
            key={s.label}
            type="button"
            onClick={() => { onChange(toISO(s.date)); onClose(); }}
      ...
      <div className="p-3 select-none">
        ...
        <div className="flex items-center justify-between mb-3 px-1">
          ...
          <div className="flex items-center gap-1">
            ...
            <button type="button" onClick={() => setViewYear(viewYear)}
        ...
        <div className="grid grid-cols-7 gap-1 mb-1">
          {DOW.map(d => <span key={d} className="text-center text-[11px] text-stone-400 dark:text-neutral-500 font-medium w-8">{d}</span>)}
        </div>
        {/* Days grid */}
...
export function NewTaskInput({ open: openProp, onOpenChange, addTaskColor, onAdd }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setTitle(''); setAssignee(''); setDueDate(''); setPriority(0);
      setOpen(false);
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div ref={containerRef} className="relative rounded-lg border border-stone-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 shadow-sm">
      {/* Title + Save */}
      ...
      <div className="border-t border-stone-100 dark:border-neutral-700">
        ...
        <div className="relative">
          <button type="button"
            ...
            {assignee ? (
              <div className="flex items-center gap-1.5">
                <div className="w-4 h-4 rounded-full bg-stone-500 flex items-center justify-center">
                  <span className="text-[7px] font-bold text-white">{assignee === 'ai' ? 'AI' : 'ME'}</span>
                </div>
            ...
            {assignee && (
              <button type="button" onClick={e => { e.stopPropagation(); setAssignee(''); }}
                className="ml-auto text-stone-400 hover:text-stone-600 text-sm leading-none">×</button>
            )}
          </button>
          {showAssigneePicker && (
            <div className="fixed z-[200] bg-white dark:bg-neutral-900 rounded-lg shadow-lg border border-stone-200 dark:border-neutral-700 py-1 w-36"
              style={{ top: popoverAnchor.top, left: popoverAnchor.left }}>
              {[{ value: 'me', label: 'Me' }, { value: 'ai', label: 'AI' }].map(a => (
        ...
        <div className="relative">
          <button type="button"
            ...
            {dueDate && (
              <button type="button" onClick={e => { e.stopPropagation(); setDueDate(''); }}
                className="ml-auto text-stone-400 hover:text-stone-600 text-sm leading-none">×</button>
            )}
          </button>
          {showDatePicker && (
        ...
        <div className="relative">
          <button type="button"
            ...
            <FlagIcon />
            {priority > 0 ? (
              <span className="text-stone-700 dark:text-neutral-300">{PRIORITIES.find(p => p.value === priority)?.label}</span>
            ) : (
              <span>Add priority</span>
            )}
            {priority > 0 && (
              <button type="button" onClick={e => { e.stopPropagation(); setPriority(0); }}
                className="ml-auto text-stone-400 hover:text-stone-600 text-sm leading-none">×</button>
            )}
          </button>
          {showPriorityPicker && (
