...
export function KanbanCard({ task, onClick }: Props) {
  ...
  return (
    <div
      ...
      <p
        className={`text-sm font-medium leading-snug ${hasFooter ? 'mb-2' : ''} ${task.done ? 'line-through text-stone-400 dark:text-neutral-500' : 'text-stone-800 dark:text-neutral-100'}`}>
        {task.title}
      </p>
      ...
      {hasFooter && (
        <div className="flex items-center gap-2">
          ...
          {task.priority > 0 && (
            <FlagIcon
              className={`ml-auto ${PRIORITY_FLAG_COLOR[task.priority] ?? 'text-stone-400'}`}
            />
          )}
        </div>
      )}
