  ...
  const schedule =
    f.scheduleKind === 'cron'
      ...
      : (() => {
          ...
          const mul = f.everyUnit === 'minutes' ? 60_000 : f.everyUnit === 'hours' ? 3_600_000 : 86_400_000;
...
export function ScheduledTaskPanel({ onOpenTask }: { onOpenTask?: (title: string) => void }) {
  ...
  const handleEdit = (job: CoreCronJob) => {
    ...
    if (s.kind === 'cron') {
      ...
      scheduleKind = 'every';
      const ms = s.every_ms;
      if (ms % 86_400_000 === 0) { everyValue = String(ms / 86_400_000); everyUnit = 'days'; }
      else if (ms % 3_600_000 === 0) { everyValue = String(ms / 3_600_000); everyUnit = 'hours'; }
      else { everyValue = String(ms / 60_000); everyUnit = 'minutes'; }
    }
    setForm({
      name: job.name ?? '',
      scheduleKind,
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <button
          onClick={() => { setFormOpen(true); setForm(EMPTY_FORM); setError(null); }}
      ...
      {error && (
        <div style={{ marginBottom: 12, padding: '8px 12px', borderRadius: 4, background: '#fff0f0', color: '#d32f2f', fontSize: 13 }}>
          {error}
        </div>
      )}
      ...
      <div style={{ display: 'flex', borderBottom: '2px solid #f0f0f0', marginBottom: 16 }}>
        {(['tasks', 'history'] as const).map(t => (
          <button key={t} onClick={() => handleTabChange(t)} style={{
            ...
            fontWeight: tab === t ? 600 : 400, color: tab === t ? '#4A83DD' : '#888',
            borderBottom: `2px solid ${tab === t ? '#4A83DD' : 'transparent'}`, marginBottom: -2,
          }}>
      ...
      {/* Create form */}
      {tab === 'tasks' && formOpen && (
        <div style={{ marginBottom: 20, border: '1px solid #c5d8f5', borderRadius: 8, padding: 16, background: '#f8fbff' }}>
          <h3 style={{ margin: '0 0 12px', fontSize: 14, fontWeight: 600 }}>{editingJobId ? 'Edit Scheduled Task' : 'New Scheduled Task'}</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <label style={{ fontSize: 13 }}>
              <div style={{ marginBottom: 4, fontWeight: 500 }}>Name <span style={{ color: '#aaa', fontWeight: 400 }}>(optional)</span></div>
              <input
                value={form.name}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {(['cron', 'every'] as const).map(k => (
                  <button
                    ...
                    style={{ padding: '4px 12px', borderRadius: 4, border: `1px solid ${form.scheduleKind === k ? '#4A83DD' : '#ddd'}`, background: form.scheduleKind === k ? '#EBF3FF' : '#fff', color: form.scheduleKind === k ? '#1967d2' : '#444', cursor: 'pointer', fontSize: 12 }}
                  >
            ...
            {form.scheduleKind === 'cron' ? (
              <label style={{ fontSize: 13 }}>
                ...
                <input
                  ...
                  placeholder="0 9 * * 1-5"
                  style={{ width: '100%', padding: '6px 10px', borderRadius: 4, border: '1px solid #ddd', fontSize: 13, fontFamily: 'monospace', boxSizing: 'border-box' }}
                />
              ...
              <label style={{ fontSize: 13 }}>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  <input
                    ...
                    style={{ width: 70, padding: '6px 10px', borderRadius: 4, border: '1px solid #ddd', fontSize: 13 }}
                  />
                  <select
            ...
            <label style={{ fontSize: 13 }}>
              <div style={{ marginBottom: 4, fontWeight: 500 }}>Task prompt <span style={{ color: '#d32f2f' }}>*</span></div>
              <textarea
                value={form.prompt}
                onChange={e => set({ prompt: e.target.value })}
                placeholder="e.g. Create a task 'Weekly sprint review' in the projects board"
                rows={3}
                style={{ width: '100%', padding: '6px 10px', borderRadius: 4, border: '1px solid #ddd', fontSize: 13, resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }}
              />
            </label>
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
            <button onClick={handleSave} disabled={saving} style={{ padding: '6px 16px', borderRadius: 4, border: 'none', background: '#4A83DD', color: '#fff', cursor: saving ? 'not-allowed' : 'pointer', fontSize: 13 }}>
              {saving ? 'Saving…' : 'Save'}
            </button>
            <button onClick={() => { setFormOpen(false); setEditingJobId(null); setForm(EMPTY_FORM); setError(null); }} style={{ padding: '6px 16px', borderRadius: 4, border: '1px solid #ddd', background: '#fff', cursor: 'pointer', fontSize: 13 }}>
              Cancel
            </button>
          </div>
        </div>
      ...
      {tab === 'history' && (
        <div>
          {historyLoading ? (
            ...
            <div style={{ color: '#888', fontSize: 13, padding: '24px 0', textAlign: 'center' }}>No runs yet.</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
                  ...
                  {history.map(r => {
                    ...
                    return (
                      <tr key={r.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                        <td style={{ padding: '8px 10px', color: '#374151', whiteSpace: 'nowrap' }}>{fmtDate(r.started_at)}</td>
                        <td style={{ padding: '8px 10px' }}>
                          <span style={{ fontSize: 11, padding: '1px 6px', background: '#dbeafe', color: '#1d4ed8', borderRadius: 4 }}>{r.job_name}</span>
                        </td>
                        <td style={{ padding: '8px 10px', color: '#6b7280' }}>{r.duration_ms ? `${(r.duration_ms / 1000).toFixed(1)}s` : '—'}</td>
                        <td style={{ padding: '8px 10px' }}>
                          <span style={{ color: r.status === 'ok' ? '#16a34a' : '#d32f2f', fontWeight: 500 }}>{r.status}</span>
                        </td>
                        <td style={{ padding: '8px 10px' }}>
                          {onOpenTask ? (
                            <button
                              onClick={() => onOpenTask(taskTitle)}
                              style={{ fontSize: 11, color: '#4A83DD', background: 'none', border: 'none', cursor: 'pointer', padding: 0, textDecoration: 'underline' }}
                            >
                              View task
                            </button>
                          ) : '—'}
              ...
              <div style={{ marginTop: 8, fontSize: 11, color: '#9ca3af' }}>{history.length} run(s)</div>
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'tasks' && (loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {jobs.map(job => (
            <div key={job.id} style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: '12px 16px', background: job.enabled ? '#fff' : '#fafafa', opacity: job.enabled ? 1 : 0.65 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <input type="checkbox" checked={job.enabled} onChange={() => handleToggle(job)} style={{ cursor: 'pointer' }} />
                  <span style={{ fontWeight: 600, fontSize: 14 }}>{job.name ?? '(unnamed)'}</span>
                  <span style={{ fontSize: 11, padding: '1px 7px', background: '#f3f4f6', color: '#6b7280', borderRadius: 10 }}>{scheduleLabel(job)}</span>
                </div>
                <div style={{ display: 'flex', gap: 6 }}>
                  <button
                    onClick={() => handleRunNow(job.id)}
                    disabled={runningId === job.id}
                    style={{ fontSize: 12, padding: '3px 10px', borderRadius: 4, border: '1px solid #c5d8f5', background: '#EBF3FF', color: '#1967d2', cursor: 'pointer' }}
                  >
                    {runningId === job.id ? 'Running…' : 'Run now'}
                  </button>
                  <button onClick={() => handleEdit(job)} style={{ fontSize: 12, padding: '3px 10px', borderRadius: 4, border: '1px solid #ddd', background: '#fff', cursor: 'pointer' }}>
                    Edit
                  </button>
                  <button onClick={() => handleDelete(job.id)} style={{ fontSize: 12, padding: '3px 10px', borderRadius: 4, border: '1px solid #ddd', background: '#fff', color: '#d32f2f', cursor: 'pointer' }}>
                    Delete
                  </button>
                </div>
              </div>
              ...
              <div style={{ marginTop: 6, display: 'flex', gap: 16, fontSize: 11, color: '#9ca3af' }}>
